...
```
src/
├── email_parser.py          # Stage 1
├── ioc_extractor.py         # Stage 2
├── ip_extractor.py          # Stage 2 — regex IP fallback
├── enrichment.py            # Stage 3
├── attachment_scanner.py    # Stage 3b
├── risk_scorer.py           # Stage 4
├── reporter.py              # Stage 5
├── slack_alert.py           # Stage 6
└── siem_logger.py           # NDJSON log writer
web/
├── app.py                   # Flask API
└── templates/index.html     # SOC dashboard
sample_emails/               # Test .eml files
output/                      # Reports + siem_logs.ndjson
...
- Rate limiting: 10 req/min, 100 req/hour per IP (`flask-limiter`)
